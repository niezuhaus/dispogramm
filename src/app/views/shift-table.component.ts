...
@Component({
  ...
  template: `
    <div>
      <table #table mat-table [dataSource]="dataSource" style="max-width: 100%;">
          ...
          <td mat-cell *matCellDef="let element" style="width: 240px">
            <div class="flex flex-row justify-content-evenly align-items-end w-100" style="width: fit-content">
              <a *ngIf="!element.edit" (click)="openShiftDialog(element)">
                <p class="text-center noMargin">{{ element.start.dateStampShort() }}, {{ shiftLiterals(element.type) }}</p>
              </a>

              <mat-form-field *ngIf="element.edit" style="width: 100px" class="relative-top8px">
                <mat-label>datum</mat-label>
                <div class="flex flex-row">
                  <input matInput [matDatepicker]="picker" [(ngModel)]="element.start" (keydown)="$event.key === 'Enter' ? updateShift(element) : ''" />
