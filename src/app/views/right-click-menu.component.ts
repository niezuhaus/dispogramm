...
@Component({
  ...
  template: `
    ...
    <!-- note -->
    <div *ngIf="item?.note">
      <button *ngIf="!item.edit" mat-menu-item (click)="item.edit = true"><i class="p-1 bi bi-pencil bi-context"></i>bearbeiten</button>
      <button mat-menu-item (click)="item.note.delete()"><i class="p-1 bi bi-trash bi-context"></i>notiz löschen</button>
    </div>

    <!-- client -->
    <div *ngIf="client">
      <button mat-menu-item [routerLink]="[routes.client, { id: client.id }]"><i class="p-1 bi bi-person bi-context"></i>kund:innenseite öffnen</button>
      <button mat-menu-item *ngIf="isDezwo" (click)="client.delete()"><i class="p-1 bi bi-trash bi-context"></i>kund:in löschen</button>
    </div>

    <!-- shift -->
    <div *ngIf="shift">
      <button mat-menu-item (click)="shift.openDialog()">
        <i class="p-1 bi bi-search bi-context"></i>
        ansehen
      </button>
      <button *ngIf="!shift.edit" mat-menu-item (click)="shift.edit = true"><i class="p-1 bi bi-pencil bi-context"></i>bearbeiten</button>
    ...
    <!-- specialprice -->
    <div *ngIf="price">
      <button mat-menu-item (click)="price.delete()"><i class="p-1 bi bi-trash bi-context"></i>sonderpreis löschen</button>
      <button mat-menu-item (click)="price.print()"><i class="p-1 bi bi-trash bi-context"></i>sonderpreis drucken</button>
    </div>
    ...
    <button *ngIf="!date && !messenger && !item?.isJob && !item?.isTemplate && !item?.isNote && !client && !shift && !price && !zone" mat-menu-item>
  ...
  openNewClientDialog(): void {
    ...
    dialog.componentInstance.saved.subscribe((client) => {
      ...
      newDialog.componentInstance.confirm.subscribe(() => {
        ...
        this.item._job
          .init()
          .save('kund:in wurde hinzugefügt')
          .subscribe(() => {
            GC.tourplan.refresh();
          });
      });
