...
export class WeekplanComponent extends TitleComponent implements OnInit, AfterViewInit {
  ...
  now = new Date();
  get todayTimeIndex() {
    return this.now.getHours().isBetween(8, 18) ? (new Date().set(8).hoursDifference(this.now) * 60) / (15).abs().floor() : -1;
  }
  week = this.today.nextWorkingDay().workingWeek();
  get weekNumber() {
    return this.week[0].workingWeekNumber();
  }
  loaded = false;
  ...
  loadWeek(): void {
    ...
    zip(this.week.map((d) => GC.http.tourplanItemsForDay(d, { onlyPlanned: true }))).subscribe((data) => {
      data.forEach((set, i) => {
        set.items.forEach((item) => {
          if (!item.regularJob?.morningTour) {
            const timeIndex = ((item._date.copy().set(8).hoursDifference(item._date) * 60) / 15).abs().floor();
  ...
  columnByDate(date: Date): { column: number; row: number } {
    return {
      ...
      row: ((new Date().set(8).hoursDifference(date) * 60) / 15).abs().floor()
    };
  }
