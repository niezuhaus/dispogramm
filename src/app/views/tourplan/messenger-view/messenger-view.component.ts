    ...
    <div>
      <div *ngFor="let messenger of list" class="grid-container">
        <div class="mr-2 flex flex-row">
          ...
          <a (click)="messenger.checkout(date)" *ngIf="messenger.shift && !messenger.shift?.end"> <i class="mr-2 bi bi-person-dash"></i>{{ messenger.nickname }} </a>
          <a (click)="messenger.checkout(date)" *ngIf="messenger.shift?.end"> <i class="mr-1 bi bi-check-lg"></i>{{ messenger.nickname }} </a>
