...
@Component({
  ...
  template: `
    <div class="flex flex-row justify-around align-items-center">
      <button *ngIf="messengersNotCheckedIn() && !item._messenger" mat-button (click)="openCheckInDialog()">kurier:innen einchecken</button>
      ...
      <searchinput
        ...
        *ngIf="!showToggle()"
        ...
        class="mx-3"
        [matTooltip]="dispatcherNotCheckedIn() ? 'bitte erst einchecken!' : ''"
      ></searchinput>
        ...
        <mat-button-toggle-group #toggle name="messenger" [value]="item._messenger?.id || ''" (change)="messengerSelected($event.value)" [disabled]="item._job?.finished || dispatcherNotCheckedIn()">
          <mat-button-toggle *ngFor="let messenger of messList" [value]="messenger.id">
            <div class="flex flex-row align-items-center">
              ...
              <span *ngIf="messenger.fexNumber" [class.active]="!item._messenger" [class.inactive]="item._messenger" class="ml-2 phoneNumber">
                {{ messenger.fexNumber }}
              </span>
            </div>
          </mat-button-toggle>
      ...
      <button
        mat-button
        (click)="resetMessenger()"
        [disabled]="dispatcherNotCheckedIn()"
        matTooltip="zurücksetzen"
        style="min-width: 40px !important; margin-left: 10px"
        *ngIf="item._messenger && !item._job?.finished"
      >
