  ...
  template: `
    <div style="min-width: 600px">
      <div class="flex flex-row justify-content-between">
        <div>
          <h1 mat-dialog-title>{{ data.shift.messenger.nickname }}s schicht vom {{ data.shift.start.dateStampLong() }}</h1>
      ...
      >
        <ng-container matColumnDef="date">
          ...
          <td mat-cell *matCellDef="let element" class="text-center">
            ...
            <timepicker [label]="'zeit'" *ngIf="element.edit" [(time)]="element.date" (keydown)="$event.key === 'Enter' ? element.update('auftrag wurde gespeichert.') : ''"></timepicker>
          </td>
        </ng-container>
          ...
          <td mat-cell *matCellDef="let element" style="color: gray;">
            <div>
              <mat-form-field>
                ...
                <input type="number" #minutes (click)="minutes.select()" matInput name="waitingMinutes" [(ngModel)]="element._waitingMinutes" (change)="tourplan.calcSales()" />
              </mat-form-field>
            </div>
          </td>
        </ng-container>
        <ng-container matColumnDef="finish">
          ...
          <td mat-cell *matCellDef="let element">
            <div class="flex flex-row justify-content-around">
              <button *ngIf="!element.finished" mat-raised-button class="fex-button" (click)="element._finished = true; addToSum(element)">tour abschließen</button>
              <button *ngIf="element.finished" mat-raised-button class="fex-button-abort" (click)="element._finished = false; subFromSum(element)" matTooltip="öffnen">
