    ...
    <div class="container">
      ...
      <mat-menu #rightMenu="matMenu">
        <ng-template matMenuContent let-price="price" let-zone="zone">
          <right-click-menu [zone]="zone" [price]="price"> </right-click-menu>
        </ng-template>
      </mat-menu>
    </div>
  `,
...
export class ConfigDialogComponent {
  ...
  save(): void {
    zip(
      Array.from(this.changedPrices.entries())
        .map((p) => {
          return this.saveConfigValue(p[0], p[1].toString());
        })
        .concat(
          Array.from(this.changedNumbers.entries()).map((n) => {
            return this.saveConfigValue(n[0], n[1].toString());
          })
        )
        .concat(
          Array.from(this.changedBooleans.entries()).map((b) => {
            return this.saveConfigValue(b[0], b[1] ? 'true' : 'false');
          })
        )
        .concat(
          Array.from(this.changedStrings.entries()).map((s) => {
