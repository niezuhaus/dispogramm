...
export class GC {
   ...
   */
  public static tourplanShiftTypes = [ShiftType.early, ShiftType.zwischi, ShiftType.late, ShiftType.double];
  /**
   *
   */
  ...
  public static loadingScreen: LoadingComponent;
  ...
  private static messengerDataToday = new Map<
    string,
    {
      jobs: Job[];
      sales: {
  ...
  private static messengerDataAnyDay = new Map<
    string,
    {
      jobs: Job[];
      sales: {
  ...
  static loadConfig(http: HttpService): Observable<boolean> {
    return http.getRawConfigItems().pipe(
      map((rcs) => {
        ...
        if (!GC.fullyLoaded) {
          GC.fullyLoaded = GC.checkLoaded(true); // config
        }
        return true;
      })
    );
  ...
  static readConfig(): ConfigDataContract {
    ...
    return {
      ...
      prices: {
        ...
        dierksenHanker: {
          base: GC.readPrice('price_dierksen_hanker_base', true) || new Price(7.5, { name: 'price_dierksen_hanker_base', brutto: true }),
          extra: GC.readPrice('price_dierksen_hanker_extra', true) || new Price(2.5, { name: 'price_dierksen_hanker_extra', brutto: true }),
          quantityIncl: 1
        },
        niemann: {
  ...
  private static loadClients(http: HttpService): Observable<boolean> {
    return http.getClientList().pipe(
      map((list) => {
        ...
        if (!GC.fullyLoaded) {
          GC.fullyLoaded = GC.checkLoaded(true); // clients
        }
        return true;
      })
    );
  ...
  private static loadLexClients(http: HttpService): void {
    ...
    http.lex_getContacts(0).subscribe({
      next: (list) => {
        ...
        GC.loadedParts.lexClients = true;
        if (!GC.fullyLoaded) {
          GC.fullyLoaded = GC.checkLoaded(true); // lex contacts
        }
      },
      error: (err) => {
        catchError(err);
  ...
  private static loadLocations = (http: HttpService) => {
    http.getLocationList().subscribe((locations) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // locations
      }
    });
  };
    ...
    http.jobsRecentDays(90).subscribe({
      next: (jobs) => {
        ...
        GC.jobsThisMonth = jobs.filter((j) => {
          ...
          return j.date.getMonth() === new Date().getMonth() && j.date.getFullYear() === new Date().getFullYear();
        });
        GC.loadedParts.jobsLastNinetyDays = true;
        GC.loadedParts.jobsThisMonth = true;
        if (!GC.fullyLoaded) {
          GC.fullyLoaded = GC.checkLoaded(true); // recent
        }
      },
      error: (msg) => {
  ...
  public static loadRegularJobs = (http: HttpService) => {
    http.getRegularJobList().subscribe((regularJobs) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // regularJobs
      }
    });
  };
  public static loadShiftsToday = (http: HttpService) => {
    http.getShiftsForToday().subscribe((list) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // shifts
      }
    });
  };
  ...
  public static loadDispatchers = (http: HttpService) => {
    http.getDispatcherList().subscribe((list) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // dispatcher
      }
    });
  };
  ...
  private static loadExpenses = (http: HttpService) => {
    http.getExpenseList().subscribe((list) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // expenses
      }
    });
  };
  ...
  public static loadSpecialPrices = (http: HttpService) => {
    http.getSpecialPriceList().subscribe((list) => {
      ...
      if (!GC.fullyLoaded) {
        GC.fullyLoaded = GC.checkLoaded(true); // specialPrices
      }
    });
  };
  ...
  public static nextClientIds = () => {
    const n =
      Math.max.apply(
        Math,
        GC.clients
          .filter((client) => parseInt(client.clientId) < 30000)
          .map((client) => {
    ...
    const b =
      Math.max.apply(
        Math,
        GC.clients
          .filter((c) => !isNaN(parseInt(c.clientId)))
          .map((client) => {
            return parseInt(client.clientId);
          })
      ) + 1;
  ...
  private static checkLoaded = (resetTimeout?: boolean) => {
    if (resetTimeout) {
      this.loadingScreen.resetTimeOutCounters();
    }
