...
export class Job extends AbstractJob {
  ...
  set _finished(value: boolean) {
    ...
    let finish = () => {
      this.finished = value;
      this.init();
      (value ? this.save('tour wurde abgeschlossen.') : this.save('abgeschlossene tour wurde wieder geöffnet.')).subscribe(() => {});
    };
  ...
  get set(): BranchSet {
    return {
      branches: this.pBranches.branches.concat(this.dBacktourBranches.branches).concat(this.dBranches.branches).concat(this.pBacktourBranches.branches)
    };
  }
  ...
  public init(options?: { pushPrice?: Price; pushDate?: Date }): Job {
    ...
    const pickups: Geolocation[] = this.pickups
      .filter((loc) => loc.locType !== LocType.centerPickup)
      .concat(
        this.deliveries
          .filter((loc) => loc.hasBacktour === true)
          .map((l) => {
            l = new Geolocation(l);
    ...
    const deliveries: Geolocation[] = this.deliveries
      .filter((loc) => loc.locType !== LocType.centerDelivery)
      .concat(
        // .filter(loc => loc.locType === LocType.delivery)
        this.pickups
          .filter((loc) => loc.hasBacktour === true)
          .map((l) => {
            l = new Geolocation(l);
  ...
  public calcDist(): number {
    let res = 0;
    this.pBranches.branches.concat(this.dBranches.branches).forEach((branch) => (res += branch.distance));
    this.pBacktourBranches.branches.concat(this.dBacktourBranches.branches).forEach((branch) => (res += branch.distance));
    return res;
  }
...
export class RegularJob extends Job {
  ...
  convert(date?: Date): Observable<Job> {
    let x = this.dates;
    let g = GC;
    let y1 = GC.tourplan.date;

    let y = x.find((d) => {
      return d && d?.getDay() === GC.tourplan.date.getDay();
    });

    return GC.http.convertRegularJob(
      this,
      date ||
        this.dates
          .find((d) => d && d?.getDay() === GC.tourplan.date.getDay())
          .copy()
