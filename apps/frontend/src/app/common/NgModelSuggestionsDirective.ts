...
@Directive({
  selector: 'textarea[type=text][ngModel][ngModelSuggestions]',
  inputs: ['suggestions: ngModelSuggestions'],
  host: {
...
export class NgModelSuggestionsDirective implements ControlValueAccessor {
  ...
  private isAcceptSuggestionEvent(event: KeyboardEvent): boolean {
    return event.key === 'Tab' || event.key === 'ArrowRight' || event.key === 'ArrowDown';
  }
}
