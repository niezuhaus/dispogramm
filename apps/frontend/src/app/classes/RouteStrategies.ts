        ...
        while (openBranch.route.filter((l) => l.passType !== PassType.nearby)[i] && i !== oldI) {
          ...
          for (let j = 0; j < route.length; ++j) {
            ...
            if (Routing.dist(openBranchLoc, location) <= GC.config.nearbyDist / 1000) {
              ...
              Routing.distPrice(Routing.priceDist(openBranch.route.filter((l) => l.passType !== PassType.nearby).concat([new Station(location), center])))._add(GC.config.prices.stop)._netto <
              ...
              Routing.distPrice(Routing.priceDist([openBranchLoc, location]))._add(Routing.distPrice(Routing.priceDist([location, center])))._netto <
              Routing.distPrice(Routing.priceDist([openBranchLoc, center]))._add(Routing.distPrice(Routing.priceDist([location, center])))._netto
            ) {
              newS = new Station(route[j]);
              dels.push(j);
